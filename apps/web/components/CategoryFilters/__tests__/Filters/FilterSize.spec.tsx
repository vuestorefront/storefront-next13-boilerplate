...
import { FilterSize } from '~/components/CategoryFilters/Filters';
...
describe('<FilterSize/>', () => {
  it('should render component with props', () => {
    const { getByText } = render(<FilterSize {...mockProps} />);

    getByText('size');
  });

  it('should render list of 5 elements', () => {
    const { container } = render(<FilterSize {...mockProps} />);
    const list = container.querySelectorAll('label');
  ...
  it('should set selected items active', () => {
    ...
    const { container } = render(<FilterSize {...props} />);
    const list = container.querySelectorAll('label');
